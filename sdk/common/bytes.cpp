...
DataBytes& DataBytes::FromBase64() {
  try {
    ...
    SetErrorAndClear(TEE_ERROR_CRYPTO_BASE64);
  }
  return *this;
}
...
DataBytes& DataBytes::ToSHA256() {
  do {
    ...
    if (!SHA256_Init(&sha256)) {
      ...
      SetErrorAndClear(TEE_ERROR_CRYPTO_SHA256);
      break;
    }
...
DataBytes& DataBytes::Randomize(const size_t len) {
  ...
  if (RAND_bytes(data(), size()) != OPENSSL_SUCCESS) {
    ...
    SetErrorAndClear(TEE_ERROR_CRYPTO_RAND);
  }
  return *this;
}
...
DataBytes& DataBytes::Export(uint8_t* buf, size_t max_buf_len) {
  if (max_buf_len < size()) {
    ...
    error_code_ = TEE_ERROR_SMALL_BUFFER;
  } else {
